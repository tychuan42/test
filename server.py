...
@app.route("/")
def index():
    return Response(INDEX_HTML, mimetype="text/html")import os
from flask import Flask, request, jsonify, Response
...
NLP_CONFIG = {
    ...
    "models": [
        {"lang_code": "en", "model_name": "en_core_web_lg"},
        {"lang_code": "sk", "model_name": "xx_ent_wiki_sm"},  # multilingválny NER pre SK
    ],
    "ner_model_configuration": {
        "model_to_presidio_entity_mapping": {
            "PER": "PERSON", "PERSON": "PERSON",
...
async function run(){
  ...
  try{
    const res=await fetch("/api/anonymize",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({text,language:$("lang").value,approach:$("approach").value,threshold:parseFloat($("threshold").value)})});
    const data=await res.json();
    if(!res.ok){showErr(data.error||"Chyba servera.");return;}
    render(data);
  }catch(e){showErr("Nepodarilo sa spojiť so serverom.");}
